"""
Turbulence XGBoost model for AE index forecasting, with Optuna hyperparameter tuning and rolling mean features.
...
# Where to save model
MODELS_DIR = pathlib.Path("models_ae_xgb_turb_new")
...
def train_eval(X_train, y_train, X_test, y_test, y_persist,
    ...
    storage = optuna.storages.JournalStorage(
        optuna.storages.JournalFileStorage("/tmp/optuna_journal_turb.log")
    )
...
def run_all(df_cut, FEATURES, TARGETS, split_time, MODELS_DIR, n_trials=60, seed=42):
    ...
    res_df.to_csv("xgb_optuna_results_turb_new.csv", index=False)
    return res_df

df_new = df_new.astype(np.float32)

res_df_turb_new = run_all(df_new, FEATURES, TARGETS, split_time, MODELS_DIR, n_trials=60, seed=42)